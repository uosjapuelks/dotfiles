require('uosjapue.core.options')
require('uosjapue.core.keymaps')
require('uosjapue.core.colorscheme')

require('uosjapue.plugins-setup')
require("uosjapue.plugins.comment")
require("uosjapue.plugins.nvim-tree")
require("uosjapue.plugins.telescope")
require("uosjapue.plugins.lualine")

require("uosjapue.plugins.nvim-cmp")
require("uosjapue.plugins.lsp.mason")
require("uosjapue.plugins.lsp.lspsaga")
require("uosjapue.plugins.lsp.lspconfig")

require("uosjapue.plugins.lsp.null-ls")
require("uosjapue.plugins.autopairs")
require("uosjapue.plugins.treesitter")
require("uosjapue.plugins.gitsigns")

require("uosjapue.plugins.nvim-dap")
require("uosjapue.plugins.nvim-ufo")
